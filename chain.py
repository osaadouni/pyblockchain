...
class Chain(object):
    ...
    def is_valid(self):
        ...
        1). Each block is indexed one after another
        2). Each block's prev_hash matches the hash of the previous block 
        3). Each block's hash is valid ( matches the number of zeros )
        '''
        ...
        for index, cur_block in enumerate(self.blocks[1:]):
            ...
            if prev_block.index+1 != cur_block.index:
                return False
            if not cur_block.is_valid(): # check the hash 
                return False
